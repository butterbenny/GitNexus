...
test('MCP action_plan: expands $this->authorize(\"account.view\") to role grants', async () => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'gitnexus-action-plan-auth-slug-'));
  const repoPath = path.join(tmpRoot, 'repo');
