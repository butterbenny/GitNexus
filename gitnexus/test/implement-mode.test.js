...
test('MCP implement_mode: returns target slice, companion files, write plan, and review handoff', async () => {
  ...
  assert.ok(result.implement_mode.write_plan.every(step => step.precedent_mapping), 'expected per-step precedent mapping');
  assert.ok(Array.isArray(result.implement_mode?.action_hints?.checks));
  assert.ok(Array.isArray(result.implement_mode?.verification_checklist));
  assert.ok(result.implement_mode.verification_checklist.length > 0);
