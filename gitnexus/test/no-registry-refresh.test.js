...
test('MCP resources: repo context reflects no-registry refresh without registry mtime changes', async () => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'gitnexus-no-registry-resource-'));
  const repoPath = path.join(tmpRoot, 'repo');
  await fs.mkdir(path.join(repoPath, 'src'), { recursive: true });
  ...
  const script = [
    ...
    'await backend.init();',
    '',
    "const beforeText = await readResource('gitnexus://repo/repo/context', backend);",
    "assert.match(beforeText, /staleness:/);",
    '',
    "// Refresh in a separate process without touching the global registry (~/.gitnexus).",
    "const out = execFileSync('node', [path.resolve('dist/cli/index.js'), 'analyze', repoPath, '--skip-embeddings', '--no-registry', '--no-hooks'], {",
    '  cwd: process.cwd(),',
    ...
    "assert.match(out, /Repository indexed successfully|Repository updated incrementally/i);",
    '',
    "const afterText = await readResource('gitnexus://repo/repo/context', backend);",
    "assert.doesNotMatch(afterText, /staleness:/);",
    '',
    'process.stdout.write(JSON.stringify({ beforeText, afterText }));',
  ].join('\n');
  ...
  const { beforeText, afterText } = JSON.parse(raw);
  assert.match(beforeText, /staleness:/);
  assert.doesNotMatch(afterText, /staleness:/);
});
