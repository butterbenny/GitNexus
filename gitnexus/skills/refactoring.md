...
4. context({name: "X"})                                     → See all incoming/outgoing refs
...
- [ ] impact({name: target, direction: "upstream"}) — find all external callers
...
- [ ] impact({name: target, direction: "upstream"}) — map callers to update
...
```
impact({name: "validateUser", direction: "upstream"})
