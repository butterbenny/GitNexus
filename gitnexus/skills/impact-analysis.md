...
```
- [ ] impact({name, direction: "upstream"}) to find dependents
...
> Disambiguation: if `impact` returns `status: ambiguous`, rerun with `uid` (preferred) or `file_path`.
...
```
1. impact({name: "validateUser", direction: "upstream"})
